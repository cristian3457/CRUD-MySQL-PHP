...
<?php	
	if(isset($_GET['editar'])){
            ...
            $usuario = $conexion->prepare("SELECT * FROM usuarios WHERE id='$editar_id'");
            $usuario->execute();
			$fila = $usuario -> fetch(PDO::FETCH_ASSOC);
...
<h1 class="text-uppercase text-center mb-3">Actualizar Usuario</h1>
...
<?php
	...
	if(isset($_POST['actualizar'])){
			...
			$actualizar = $conexion->prepare("UPDATE usuarios SET usuario='$actualizar_nombre', password='$actualizar_password', email='$actualizar_email' WHERE id='$editar_id'");
			$ejecutar = $actualizar->execute();
			if($ejecutar){
				$tabla = 'usuarios';
				$campo = 'img';
